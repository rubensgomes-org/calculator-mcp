...
# GitHub API endpoints and request settings
readonly GITHUB_RATE_LIMIT_URL="https://api.github.com/rate_limit"
readonly GITHUB_API_VERSION="2022-11-28"
readonly CURL_TIMEOUT_SECS=10
...
#
# Retrieves the GitHub token held by the gh CLI
#
# Arguments:
#   None
# Returns:
#   0 if a token is available, 1 if not
# Side Effects:
...
#
# Performs a GET against the GitHub REST API, authenticating with the gh CLI
# token when one is available so that private and internal repositories
# resolve instead of returning 404
#
# Arguments:
#   $1 - API URL to request
# Returns:
#   curl exit status
# Side Effects:
...
github_api_get() {
    ...
    if ! token=$(github_token); then
        curl -s --max-time "${CURL_TIMEOUT_SECS}" "${url}"
        return
    fi
...
#
# Reads a field from a JSON document
#
# Arguments:
#   $1 - JSON document
#   $2 - jq filter identifying the field
# Returns:
#   0 if the field is present and not null, 1 if not
# Side Effects:
...
check_dependencies() {
    ...
    local -a required_commands=(
        "curl" "git" "host" "timeout" "grep" "awk" "jq"
    )
    ...
    if [ ${#missing_deps[@]} -gt 0 ]; then
        ...
        echo "  Ubuntu/Debian: sudo apt-get install curl git dnsutils coreutils grep gawk jq"
        echo "  RHEL/CentOS:   sudo yum install curl git bind-utils coreutils grep gawk jq"
        echo ""
        return 1
    fi
...
test_https_connectivity() {
    ...
    if curl -s -I --max-time "${CURL_TIMEOUT_SECS}" https://github.com | head -1 | grep -q "200\|301\|302"; then
...
#
# Test 3: Repository Accessibility via REST API
# Checks if the repository can be accessed through GitHub's REST API,
# authenticated when a gh CLI token is available
#
# Arguments:
#   None
# Returns:
...
test_repository_api_access() {
    echo "Test 3: Repository Accessibility (REST API)"

    local repo_info
    repo_info=$(github_api_get "${GITHUB_API_URL}")

    local full_name
    if full_name=$(json_field "${repo_info}" '.full_name'); then
        echo -e "${GREEN}✓ PASSED${NC} - Repository accessible via API"
        echo "  Repository: ${full_name}"
        echo "  Private: $(json_field "${repo_info}" '.private')"
...
test_api_rate_limits() {
    ...
    local rate_limit
    rate_limit=$(github_api_get "${GITHUB_RATE_LIMIT_URL}")

    local limit
    if limit=$(json_field "${rate_limit}" '.resources.core.limit'); then
        local remaining
        remaining=$(json_field "${rate_limit}" '.resources.core.remaining')
...
test_latest_release() {
    ...
    local release_info
    release_info=$(github_api_get "${releases_url}")

    local latest_tag
    if latest_tag=$(json_field "${release_info}" '.tag_name'); then
